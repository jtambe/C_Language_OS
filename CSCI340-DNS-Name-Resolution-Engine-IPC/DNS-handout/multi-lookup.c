...
int main(int argc, char* argv[])
{
	...
	*counter = 0;
	...
	int resolveThreads[argc];
	int i;
	// opening minimum 2 processes for resolving
	// since argc minimum value is 2
  for(i =0 ;i < (argc); i++)
  {
		// get pids for waitpid command
		if((resolveThreads[i] = fork()) == 0)
		{
			// open new process work
...
void* RequestHostNames(void* voidptr)
{
  ...
  // get hostname from input file
	while(fscanf(inputfp, INPUTFS, hostname) > 0)
	{
		...
		(*counter) = (*counter) +1;
		//fprintf(stderr,"&(hostArray[tempcount* SBUFSIZE]: %s from RequestHostNames at %d \n", &(hostArray[tempcount* SBUFSIZE]),*counter -1);
